  ...
  return (
    <Card className={styles.CardWrapper}>  // Use className from styles object correctly
      <CardMedia component="div">
        <Img fluid={img} alt={title} className={styles.img}/> // Ensure CSS classes are referenced correctly
      </CardMedia>
      <CardContent sx={{ textAlign: 'left' }}>
        <Typography variant="body2" color="textSecondary">
          <span className={styles.Type}>{type}</span> · {formattedDate} // Use className from styles for `type`
        </Typography>
